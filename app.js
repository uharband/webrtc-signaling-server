...
function getConnection() {
    ...
    request.get(serverUri + 'application/queue', (err, res, body) => {
        if (res.statusCode !== 200 || err) {setTimeout(getConnection, 2000); return console.log(body); }
        var response = JSON.parse(body)
        console.log("Got a connection id from the signaling server id: " + response.connectionId);
        setTimeout(getOffer, 2000, response.connectionId);
    });
}
